...
def net(input_size=(IMAGE_SIZE, IMAGE_SIZE, 3), num_class=20):
    ...
    sub_net_1 = LHD(inputs, filters)
    sub_net_2 = LUNET(inputs, filters)

    sub_out_1 = Conv2D(num_class, 1, activation='softmax', name='LHD_out')(sub_net_1)
    sub_out_2 = Conv2D(num_class, 1, activation='softmax', name='LUNET_out')(sub_net_2)

    model = Model(inputs=inputs, outputs=[sub_out_1, sub_out_2])

    model.compile(loss={'LHD_out': [LHD_loss],
                        'LUNET_out': [LUNET_loss]},
                  loss_weights={
                      'LHD_out': 1.,
                      'LUNET_out': 1.
                  },
